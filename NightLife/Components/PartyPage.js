...
import { Text, View, Dimensions, Image,TextInput,AsyncStorage,TouchableOpacity,ImageBackground,Animated} from 'react-native';
import styles from './pageStyle';
import {Button,ThemeProvider,CheckBox,Header} from 'react-native-elements';
...
componentDidMount(){
    ...
    if(this.state.places!=null){
      debugger;
       markers =  this.state.places.map((place, index) => {
        ...
        return(
        <Marker
        coordinate={{latitude:parseFloat(place.Lati),
                     longitude:parseFloat(place.Longi)}}
        onPress={()=>{this.infoWindow(place,index)}}
       key={index}
      >
  ...
  //           </View>
    ...
    <View style={{backgroundColor: 'rgba(255,255,255,.6)',height:'100%'}}>
              ...
              <TouchableOpacity  onPress={()=>{<View><Text>05555555</Text></View>}} success type="outline" >
